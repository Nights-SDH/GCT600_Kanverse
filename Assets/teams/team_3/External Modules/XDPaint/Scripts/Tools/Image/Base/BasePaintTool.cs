...
namespace XDPaint.Tools.Image.Base
{
    ...
    public abstract class BasePaintTool<T> : IPaintTool where T : BasePaintToolSettings
    {
        ...
        {
            var fillColor = QualitySettings.activeColorSpace == ColorSpace.Linear ? color.linear : color;
            Data.CommandBuilder.Clear().LoadOrtho().SetRenderTarget(GetTarget(RenderTarget.ActiveLayer)).ClearRenderTarget(fillColor).Execute();
